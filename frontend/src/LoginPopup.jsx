      ...
      if (err.response?.data?.detail) {
        if (Array.isArray(err.response.data.detail)) {
          ...
          if (errorMessage.includes('verify your email')) {
            setError('Please check your email and click the verification link before logging in. If you didn\'t receive the email, check your spam folder.');
          } else {
            setError(errorMessage);
          }
          
          // Temporarily prevent closing the modal when there's an error
          setAllowClose(false);
          setTimeout(() => setAllowClose(true), 3000); // Allow closing after 3 seconds
        } else {
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 z-10 relative animate-fadeIn">
          ...
          <button 
            ...
            className={`absolute top-6 right-6 text-gray-400 hover:text-indigo-600 transition-colors ${
              !allowClose ? 'opacity-50 cursor-not-allowed' : ''
            }`}
            disabled={!allowClose}
          >
            <X className="h-6 w-6" />
